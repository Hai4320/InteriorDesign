...
description: >
  ...
  designs/<slug>/00-project.yaml + 03-layout.md. Output: 05-budget.md.
---
...
Đọc `00-project.yaml` và `03-layout.md`. **Thiếu layout → dừng**, báo chạy `/interior-layout` (dự toán phải bóc từ danh sách đồ đã chốt, không đoán). Validate `00-project.yaml` theo `../interior/references/schema.md` trước khi chạy. Đọc khoảng giá từ `../interior/references/price-ranges-vn.md`. Khung mẫu output: `../interior/assets/05-budget.md`.
...
Trong `05-budget.md` thêm 3 checklist:
...
Tạo `05-budget.md` (bảng + so sánh ngân sách + 3 checklist). Cập nhật `status.budget: done`. Nếu bước 1–5 đã `done`: chúc mừng, tóm tắt bộ hồ sơ trong `designs/<slug>/` và gợi ý `/interior-present` để gộp tất cả thành design board HTML xem/in/share.

## Gate trước khi set `status.budget: done`

Tự kiểm trước khi set done: (1) **mọi dòng tổng đúng số học** (bất biến §7.3 — cộng lại bằng tay từng nhóm + tổng cuối); (2) **đồ trong bảng (trừ `keep_items`) ⊆ danh sách đồ `03-layout.md`** (§7.2, không phát sinh món ngoài layout); (3) chạy `python3 .claude/skills/interior/scripts/check_project.py designs/<slug>/`. Sai số học hoặc đồ lạ → sửa trước khi `done`.
