...
Đọc `00-project.yaml`, `02-concept.md`, `03-layout.md` của dự án. **Thiếu concept → dừng**, báo chạy `/interior-concept`. Validate `00-project.yaml` theo `../interior/references/schema.md` trước khi chạy. Thiếu layout: vẫn làm được nhưng cảnh báo ảnh sẽ không khớp bố cục thật, khuyên chạy `/interior-layout` trước.
...
## Gate trước khi set `status.render: done`

Tự kiểm: mọi mã hex trong scene facts khớp `concept.palette`; vị trí cửa sổ/đồ trong prompt khớp `03-layout.md` (nếu có); ánh sáng nêu đúng `room.sun`. Lệch → sửa prompt trước khi `done`.
