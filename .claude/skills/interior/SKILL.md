...
| 5 | `interior-budget` | Dự toán + shopping list + checklist | `05-du-toan.md` |
...
3. **Có dự án**: đọc `status` trong `00-project.yaml` từng dự án, in bảng trạng thái:
   ...
   Nhiều dự án: hỏi user muốn làm tiếp cái nào. Sau đó mời vào bước `pending` đầu tiên theo thứ tự pipeline, nêu rõ skill cần gọi (vd "bước kế: `/interior-layout`").
...
- Reference data dùng chung cho cả bộ nằm tại `.claude/skills/interior/references/` (ergonomics, styles, furniture-sizes, price-ranges-vn).
